...
gym.logger.set_level(logging.CRITICAL)  # Suppress gym logs
...
checkpoint_callback = None  # Skip periodic model saving
eval_callback = None  # Disable evaluation during training

class LoggingMode(Enum):
    #INFO = logging.INFO
    #DEBUG = logging.DEBUG
    ERROR = logging.ERROR
logging.basicConfig(level=logging.ERROR)  # Apply globally
...
def run_stable_baselines_argsparser():
    ...
    parser.add_argument("--logging_mode", default=LoggingMode.ERROR, type=LoggingMode, choices=list(LoggingMode))
...
def construct_logger(logging_mode: int, save_path: str, logger):
    ...
    if logging_mode == logging.ERROR:  # Only log critical issues
        logger.setLevel(logging.ERROR)
    else:
        logger.setLevel(logging.CRITICAL)  # Disable all logs
       
    
    
    stream_handler = logging.StreamHandler()
    #stream_handler.setLevel(logging_mode)
    stream_handler.setLevel(logging.CRITICAL)  # No unnecessary logs
    ...
    logger.debug("Logging is disabled for performance.")
...
def continue_learning(hyperparams, args, env, tensorboard_log, callbacks, normalize, save_path, env_id, rank):
    ...
    #if rank == 0:
    if rank == 0 and args.save_freq > 0:  # Only save if save_freq > 0
...
def optimize_parameters(hyperparams, args, save_path, create_env, sampling_setting_reward_configs,
    ...
    def create_model(hyperparams, configs):
        ...
        env = create_env(args.n_envs, eval_env=False, **configs)
        # Debug print for action space
        print("DEBUG: Env =", env)
        print("DEBUG: Env class =", env.__class__.__name__)
        print("DEBUG (train_model.py): Training env action space =", env.action_space)
        print("DEBUG (train_model.py): Training env action space type =", type(env.action_space))
        ...
        #return ALGOS[args.algo](env=create_env(args.n_envs, eval_env=False, **configs),
         #                       tensorboard_log=None, verbose=0, **hyperparams)#tensorboard_log=tensorboard_log, verbose=1
    ...
    if args.optimize_hyperparams:
        ...
        optimized_hyperparams = optimize_hyperparams(args.algo,
                                                     ...
                                                     best_model_save_path=None)#log_path)
...
def train_from_scratch(hyperparams, args, env, tensorboard_log, callbacks, save_path, env_id, normalize, rank):
    ...
    if args.algo == 'gail':
        ...
        model = ALGOS[args.algo](env=env, tensorboard_log=None,
                             seed=args.seed, verbose=0, **hyperparams)#verbose=args.logging_mode.value
...
def run_stable_baselines(args):
    ...
    save_path = None  # or skip entirely

    #construct_logger(args.logging_mode.value, save_path, LOGGER)
    #construct_logger(logging.ERROR, save_path, LOGGER)  # Only log errors
    construct_logger(logging.CRITICAL, None, LOGGER)  # Suppress everything
    ...
    logger.configure(folder=None)  # Disable SB3 logger
    ...
    if mpi4py is not None and MPI.COMM_WORLD.Get_size() > 1:
        ...
        if rank != 0:
            ...
            LOGGER.setLevel(logging.CRITICAL)  # Disable logs entirely
            args.tensorboard_log = None  # Completely disable TensorBoard logging
    ...
    if args.optimize_hyperparams or args.optimize_observation_configs or args.optimize_reward_configs:
        ...
        if args.eval_freq > 0:
            ...
            eval_callback = MultiEnvsEvalCallback(
                ...
                log_path=save_path,
                best_model_save_path=None,#best_model_save_path=save_path
                eval_freq=eval_freq,
                n_eval_timesteps=args.eval_timesteps,
                callback_on_new_best=None,#callback_on_new_best=save_vec_normalize,
                verbose=None,#verbose=args.logging_mode.value
            )
