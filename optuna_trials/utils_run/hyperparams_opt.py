...
def log_memory_usage():
    ...
    LOGGER.info(f"Memory usage: {process.memory_info().rss / 1024 ** 3:.2f} GB")
###
'''


def clear_tf_session():
    """Clears TensorFlow session to release memory."""
...
def init_enum_type():
    # Check for a "lock file" to know if this has been done already
    if os.path.exists("/tmp/optuna_enum_initialized"):
        return  # Someone else already created/dropped the ENUM — skip!

    # Connect to PostgreSQL
    engine = create_engine("postgresql://optuna:pass@localhost:5432/optunadb")
    with engine.connect() as conn:
        conn.execute(text("""
        DO $$ BEGIN 
            IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'studydirection') 
            THEN 
                DROP TYPE studydirection CASCADE; 
            END IF; 
        END $$;
        """))

    # Create the lock file so other processes skip this next time
    open("/tmp/optuna_enum_initialized", "w").close()    
...
def optimize_hyperparams(
    ...
    # PostgreSQL load study
      
    storage_url = os.environ.get("OPTUNA_STORAGE")  
    study_name = os.environ.get("OPTUNA_STUDY_NAME") 
    
    if storage_url is None or study_name is None:
    	raise ValueError("OPTUNA_STORAGE and OPTUNA_STUDY_NAME must be set!")   
    ...
    def objective_hyperparams(trial):
        ...
        """
        #modified
        #log_memory_usage()  # Log memory usage before starting the trial
        ###
	...
	"""
        ...
        hyperparams_eval_callback = HyperparamsTrialEvalCallback(
            ...
            best_model_save_path=None,#best_model_save_path
            deterministic=True,
            verbose=0,#verbose
        )
        if algo == "her":
        ...
        try:
            ...
            LOGGER.warning("Trial failed due to NaN or invalid hyperparameters.")
            raise optuna.exceptions.TrialPruned()  # <- THIS IS CRUCIAL


        # Restore stdout after training
    ...
    # Manually clear the rollout buffer
        if hasattr(model, "rollout_buffer"):
            del model.rollout_buffer  # PPO2 stores rollout data here
        
        
        del model.env, eval_env
        del model
        
        clear_tf_session()  # Reset TensorFlow to release memory
                
        # Collect garbage and log memory usage
        gc.collect()
       # log_memory_usage()
...
def sample_ppo2_params(trial):
    ...
    cliprange_vf = trial.suggest_float("cliprange_vf", 0.0, 0.5)

    
    
    
    # Ensure batch size does not exceed n_steps
...
def sample_sac_params(trial):
    ...
    learning_rate = trial.suggest_loguniform("lr", 1e-5, 1)
    #added tau
    tau = trial.suggest_loguniform("tau", 1e-4, 1.0)  # polyak update
   
    batch_size = trial.suggest_categorical("batch_size", [16, 32, 64, 128, 256, 512])
    buffer_size = trial.suggest_categorical(
        "buffer_size", [int(1e4), int(1e5), int(1e6)]
    )
    #buffer_size = trial.suggest_int("buffer_size", int(5e3), int(5e7), log=True)

    #original 
    #learning_starts = trial.suggest_categorical("learning_starts", [0, 1000, 10000, 20000])
    #modified
    learning_starts = trial.suggest_int("learning_starts", 0, 10000)

    #original train_freq range
    #train_freq = trial.suggest_categorical("train_freq", [1, 10, 100, 300])
    #modified 
    train_freq = trial.suggest_int("train_freq", 1, 1000)
    
    # gradient_steps takes too much time
    gradient_steps = trial.suggest_categorical('gradient_steps', [1, 10, 100, 300])
    #gradient_steps = train_freq
    #modified added 1.0 nad replaced 0.0001 with 0.001
    ent_coef = trial.suggest_categorical(
        "ent_coef", ["auto", 1.0, 0.5, 0.1, 0.05, 0.01, 0.001]
    )
